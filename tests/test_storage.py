...
class BaiduStoragePureMethodTests(unittest.TestCase):
    ...
    def test_transfer_count_limit_error_detection(self):
        ...
        self.assertTrue(
            is_transfer_count_limit_error("error_code: 4, message: share transfer pcs error")
        )
...
class BaiduClientAdapterRetryTests(unittest.TestCase):
    ...
    def test_retry_debug_log_masks_raw_error_message(self):
        ...
        with (
            patch("storage_client.time.sleep"),
            patch("storage_client.get_logger", return_value=logger),
        ):
            result = adapter.list("/save")
    ...
    def test_transfer_shared_paths_does_not_retry_in_adapter(self):
        ...
        adapter.client.transfer_shared_paths.assert_called_once_with(remotedir="/save", fs_ids=[1])
...
class SharedPathServiceTests(unittest.TestCase):
    ...
    def test_resolve_shared_root_uses_common_parent(self):
        ...
        self.assertEqual("/sharelink123-456", self.service._resolve_shared_root(shared_paths))

    def test_trim_shared_root_keeps_relative_structure(self):
        self.assertEqual(
            "电影/单集.mp4",
            self.service._trim_shared_root("/sharelink123-456/电影/单集.mp4", "/sharelink123-456"),
        )
        self.assertEqual(
            "单集.mp4",
            self.service._trim_shared_root("/single-share/单集.mp4", "/single-share"),
        )
    ...
    def test_list_shared_files_reports_throttled_progress(self):
        ...
        self.assertTrue(
            any(
                "共享文件列表获取完成：扫描 1 个目录 / 1 页，发现 1 个文件" in msg
                for msg in messages
            )
        )
    ...
    def test_list_shared_files_skips_excluded_node_modules(self):
        ...
        files = self.service.list_shared_files([root_dir], exclude_folder_filter=r"^node_modules$")
    ...
    def test_list_shared_files_handles_deep_directory_without_recursion(self):
        ...
        listings[path] = [
            SimpleNamespace(
                path=f"{path}/file.txt", is_dir=False, fs_id=99, size=123, md5="md5-file"
            )
        ]
        self.service.client.list_shared_paths.side_effect = lambda scan_path, *args, **kwargs: (
            listings[scan_path]
        )
    ...
    def test_list_shared_files_excludes_deep_filtered_directory_without_recursion(self):
        ...
        app_file = SimpleNamespace(
            path="/single-share/src/app.py", is_dir=False, fs_id=12, size=1, md5="app"
        )
        listings = {
    ...
    def test_list_shared_dir_children_returns_direct_child_metadata(self):
        ...
        children = self.service.list_shared_dir_children("/single-share", 1, 2, "token")
    ...
    def test_iter_shared_dir_children_yields_multiple_pages(self):
        ...
        with patch.object(SharedPathService, "SHARED_DIR_PAGE_SIZE", 1):
            children = list(self.service.iter_shared_dir_children("/single-share", 1, 2, "token"))
...
class StoragePathServiceTests(unittest.TestCase):
    ...
    def test_list_local_files_in_dirs_merges_sibling_dirs_when_enabled(self):
        ...
        listings = {
            "/save/A": [
                SimpleNamespace(
                    is_file=True, is_dir=False, path="/save/A/root.txt", md5="md5-root"
                ),
        ...
        result = service.list_local_files_in_dirs("/save", {"A/1", "A/2"}, merge_dirs=True)
        ...
        self.assertEqual(
            [call("/save/A"), call("/save/A/1"), call("/save/A/2")], client.list.call_args_list
        )
    ...
    def test_list_local_files_in_dirs_cache_separates_merge_mode(self):
        ...
        direct_result = service.list_local_files_in_dirs("/save", {"A/1", "A/2"}, use_cache=True)
    ...
    def test_ensure_dir_exists_retries_transient_empty_response(self):
        ...
        with (
            patch("storage_paths.time.sleep") as sleep,
            patch("storage_paths.handle_error_and_notify") as notify,
        ):
    ...
    def test_list_local_files_in_dirs_handles_deep_merged_scan_without_recursion(self):
        ...
        listings = {
            ...
            "/save/A/other": [
                SimpleNamespace(
                    is_file=True, is_dir=False, path="/save/A/other/ignored.txt", md5="ignored"
                )
            ],
        }
        path = "/save/A/1"
    ...
    def test_list_local_files_in_dirs_concurrent_propagates_worker_exception(self):
        ...
        with (
            patch("storage_paths.LOCAL_SCAN_CONCURRENCY", 2),
            patch("storage_paths.handle_error_and_notify"),
        ):
            with self.assertRaises(RuntimeError):
...
class WeChatNotifierTests(unittest.TestCase):
    ...
    def test_send_message_masks_sensitive_payload_before_posting(self):
        ...
        with (
            patch("wechat_notifier.requests.post", return_value=response) as post,
...
class BaiduStorageFlowTests(unittest.TestCase):
    ...
    def test_transfer_share_returns_dir_error_directly(self):
        ...
        self.storage._load_share_entries = Mock(return_value=entry_context)
        self.storage._load_share_files = Mock()
        self.storage.share_service.iter_shared_files.return_value = [{"fs_id": 1, "path": "a.txt"}]
        self.storage._scan_local_files_dict = Mock(return_value={})
    ...
    def test_transfer_share_streams_transfer_before_scan_finishes(self):
        ...
        self.storage._load_share_entries = Mock(return_value=entry_context)
        self.storage._load_share_files = Mock()
        self.storage.share_service.iter_shared_files.side_effect = lambda *args, **kwargs: (
            shared_files()
        )
        self.storage._scan_local_files_dict = Mock(return_value={})
    ...
    def test_transfer_share_streaming_preserves_regex_rename(self):
        ...
        self.storage.client.rename.assert_called_once_with("/save/old.mp4", "/save/new.mp4")
    ...
    def test_transfer_share_streaming_warns_when_producer_thread_stays_alive(self):
        ...
        with (
            patch("storage_streaming.threading.Thread", FakeThread),
            patch("storage_streaming.get_logger", return_value=fake_logger),
    ...
    def test_transfer_share_checks_target_exists_before_retrying_dir_fast_path(self):
        ...
        self.storage._transfer_share_streaming = Mock(
            return_value={"success": True, "message": "fallback"}
        )
    ...
    def test_transfer_share_divides_tree_when_root_dir_hits_count_limit(self):
        ...
        child_file = {
            "raw": SimpleNamespace(
                path="/share/course/a.txt", is_dir=False, is_file=True, fs_id=11
            ),
            "fs_id": 11,
    ...
    def test_transfer_share_recurses_when_subdir_fast_path_hits_count_limit(self):
        ...
        big_child = SimpleNamespace(
            path="/share/course/big/a.txt", is_dir=False, is_file=True, fs_id=21
        )
        child_dir = {
            "raw": big_dir,
            "fs_id": 20,
    ...
    def test_transfer_share_uses_tree_divide_when_exclude_filter_is_set(self):
        ...
        excluded_dir = {
            "raw": SimpleNamespace(
                path="/share/course/node_modules", is_dir=True, is_file=False, fs_id=20
            ),
            "fs_id": 20,
        ...
        app_file = {
            "raw": SimpleNamespace(
                path="/share/course/app.py", is_dir=False, is_file=True, fs_id=21
            ),
            "fs_id": 21,
    ...
    def test_transfer_dir_tree_divide_handles_deep_directory_without_recursion(self):
        ...
        for index in range(depth):
            child_path = f"{path}/d{index}"
            child_dir = SimpleNamespace(
                path=child_path, is_dir=True, is_file=False, fs_id=index + 20
            )
        ...
        children_by_path[path] = [
            {
                "raw": SimpleNamespace(
                    path=f"{path}/file.txt", is_dir=False, is_file=True, fs_id=999
                ),
                "fs_id": 999,
        ...
        self.storage.client.transfer_shared_paths.assert_called_once()
        self.assertEqual(
            [999], self.storage.client.transfer_shared_paths.call_args.kwargs["fs_ids"]
        )
    ...
    def test_transfer_dir_tree_divide_splits_files_by_batch_size(self):
        ...
        children = [
            {
                "raw": SimpleNamespace(
                    path=f"/share/course/{fs_id}.txt", is_dir=False, is_file=True, fs_id=fs_id
                ),
                "fs_id": fs_id,
    ...
    def test_process_single_share_config_handles_invalid_config(self):
        ...
        with patch("storage.handle_error_and_notify"):
            result = self.storage._process_single_share_config(
                1, 2, "bad-config", progress_callback
            )
    ...
    def test_transfer_multiple_shares_concurrent_preserves_index_order(self):
        ...
        self.assertEqual([1, 2, 3], [item["index"] for item in result["results"]])
    ...
    def test_build_transfer_list_skips_rename_candidate_when_source_exists_to_avoid_duplicate_copy(
        self,
    ):
        self.storage.path_service.normalize_path.side_effect = lambda path, file_only=False: (
            path.strip("/")
    ...
    def test_execute_transfer_plan_retries_count_limit_batch_with_smaller_batches(self):
        self.storage.path_service.normalize_path.side_effect = lambda path, **kwargs: path
        transfer_list = [
            (fs_id, "/save", f"{fs_id}.txt", f"{fs_id}.txt", False) for fs_id in range(350)
        ]
        self.storage.client.transfer_shared_paths.side_effect = [
        ...
        with (
            patch("storage_transfer_plan.TRANSFER_BATCH_SIZE", 999),
            patch("storage_transfer_plan.time.sleep"),
    ...
    def test_execute_transfer_plan_isolates_retryable_batch_failures(self):
        ...
        self.storage._scan_local_files_dict = Mock(return_value={})
        transfer_list = [
            (fs_id, "/save", f"{fs_id}.txt", f"{fs_id}.txt", False) for fs_id in range(3)
        ]
        ...
        with (
            patch("storage_transfer_plan.TRANSFER_BATCH_SIZE", 3),
            patch("storage_transfer_plan.TRANSFER_FAILED_RETRY_ATTEMPTS", 1),
            patch("storage_transfer_plan.time.sleep"),
            patch("storage_transfer_plan.handle_error_and_notify") as notify,
        ):
            success_count, successful_items, failed_items = self.storage._execute_transfer_plan(
    ...
    def test_execute_transfer_plan_does_not_hot_retry_storage_temporary_errors(self):
        ...
        self.storage._verify_existing_for_batch = Mock(return_value=([], [transfer_item]))

        with (
            patch("storage_transfer_plan.TRANSFER_FAILED_RETRY_ATTEMPTS", 5),
            patch("storage_transfer_plan.time.sleep") as sleep,
            patch("storage_transfer_plan.handle_error_and_notify") as notify,
        ):
            success_count, successful_items, failed_items = self.storage._execute_transfer_plan(
                [transfer_item], "url", 1, 2, "token", "/save"
    ...
    def test_execute_transfer_plan_stops_remaining_batches_on_storage_temporary_error(self):
        ...
        self.storage._verify_existing_for_batch = Mock(return_value=([], [transfer_list[0]]))

        with (
            patch("storage_transfer_plan.TRANSFER_BATCH_SIZE", 1),
            patch("storage_transfer_plan.TRANSFER_FAILED_RETRY_ATTEMPTS", 5),
            patch("storage_transfer_plan.time.sleep") as sleep,
    ...
    def test_execute_transfer_plan_retries_failed_items_only(self):
        ...
        self.storage._verify_existing_for_batch = Mock(return_value=([], [transfer_list[1]]))
    ...
    def test_execute_transfer_plan_does_not_retry_non_retryable_errors(self):
        self.storage.path_service.normalize_path.side_effect = lambda path, **kwargs: path
        transfer_item = (1, "/save", "a.txt", "a.txt", False)
        self.storage.client.transfer_shared_paths.side_effect = RuntimeError("error_code: 31066")
        self.storage._verify_existing_for_batch = Mock(return_value=([], [transfer_item]))
    ...
    def test_execute_transfer_plan_clears_failed_record_when_retry_finds_final_path(self):
        ...
        self.storage._split_existing_transfer_items = Mock(return_value=([], [transfer_item]))
        self.storage._verify_existing_for_batch = Mock(
    ...
    def test_execute_transfer_plan_renames_existing_source_without_retransfer(self):
        self.storage.path_service.normalize_path.side_effect = lambda path, **kwargs: path
        transfer_item = TransferItem(1, "/save/old", "old/a.txt", "new/a.txt", True, "md5-a")
    ...
    def test_build_transfer_result_includes_transfer_failed_files(self):
        failed_files = [{"fs_id": 2, "clean_path": "b.txt", "final_path": "b.txt", "error": "boom"}]
    ...
    def test_build_transfer_result_treats_transfer_success_with_all_rename_failures_as_partial(
        self,
    ):
        result = self.storage._build_transfer_result(
            2,
...
class BaiduClientAdapterTests(unittest.TestCase):
    def test_parse_cookies_skips_invalid_items(self):
        result = BaiduClientAdapter.parse_cookies("BDUSS=foo; invalid; STOKEN=bar; key = value ")
...
class BaiduClientAdapterPoolSizingTests(unittest.TestCase):
    ...
    def test_compute_pool_maxsize_explicit_value_ignores_ga_offset(self):
        adapter = self._adapter(is_github_actions=True)
        with patch.dict(os.environ, {"TRANSFERSHARE_PCS_POOL_MAXSIZE": "64"}, clear=True):
    ...
    def test_compute_pool_maxsize_zero_signals_disable(self):
        adapter = self._adapter(is_github_actions=True)
        with patch.dict(os.environ, {"TRANSFERSHARE_PCS_POOL_MAXSIZE": "0"}, clear=True):
            maxsize, connections, fanout = adapter._compute_pool_maxsize()
        # 显式 0：表示禁用 patch
    ...
    def test_compute_pool_maxsize_invalid_fanout_falls_back(self):
        adapter = self._adapter(is_github_actions=False)
        with patch.dict(os.environ, {"TRANSFERSHARE_PCS_POOL_FANOUT": "bad"}, clear=True):
            with self.assertLogs("transfershare", level="WARNING") as ctx:
                _, _, fanout = adapter._compute_pool_maxsize()
        self.assertEqual(4, fanout)
        self.assertTrue(any("TRANSFERSHARE_PCS_POOL_FANOUT" in msg for msg in ctx.output))
...
class BaiduClientAdapterSessionPatchTests(unittest.TestCase):
    ...
    def test_patch_session_pool_replaces_https_and_http_adapters(self):
        ...
        self.assertEqual(32, new_https.poolmanager.connection_pool_kw.get("maxsize"))
        self.assertEqual(32, new_http.poolmanager.connection_pool_kw.get("maxsize"))
    ...
    def test_patch_session_pool_skips_when_session_is_not_requests_session(self):
        ...
        self.assertTrue(any("不是 requests.Session 实例" in msg for msg in ctx.output))
    ...
    def test_patch_session_pool_rolls_back_on_mount_failure(self):
        ...
        with (
            ...
            patch.object(pcs_candidate._session, "mount", side_effect=faulty_mount),
        ...
        self.assertFalse(adapter.session_pool_info["patched"])
        self.assertTrue(any("会话连接池替换失败" in msg for msg in ctx.output))
    ...
    def test_apply_session_patches_skips_when_already_patched(self):
        ...
        with (
            patch.dict(os.environ, {}, clear=True),
            patch.object(adapter, "_patch_session_pool") as mock_pool,
            self.assertLogs("transfershare", level="DEBUG") as ctx,
        ):
    ...
    def test_apply_session_patches_is_idempotent(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            with patch.object(pcs_candidate._session, "mount") as mock_mount:
                adapter._apply_session_patches(pcs_candidate)
        # 第二次完全没调 mount
    ...
    def test_apply_session_patches_logs_info_with_status(self):
        ...
        with (
            patch.dict(os.environ, {}, clear=True),
            self.assertLogs("transfershare", level="INFO") as ctx,
        ):
            adapter._apply_session_patches(pcs_candidate)
    ...
    def test_init_client_keeps_working_when_session_pool_patch_fails(self):
        ...
        with (
            patch.object(BaiduClientAdapter, "_pcs_factory", staticmethod(FakeApi)),
            patch.dict(os.environ, {}, clear=True),
        ):
...
class SharedPathServicePaginationSafetyTests(unittest.TestCase):
    def setUp(self):
        self.service = SharedPathService(Mock())
    ...
    @patch.dict(
        "os.environ",
        {
            "TRANSFERSHARE_SHARED_PAGE_SIZE": "2",
            "TRANSFERSHARE_SHARED_PAGE_PROBE_ON_SHORT": "1",
        },
        clear=False,
    )
    def test_probe_on_short_does_not_stop_when_server_caps_page(self):
        self.service.SHARED_DIR_PAGE_SIZE = 2
        self.service.SHARED_DIR_PAGE_PROBE_ON_SHORT = True
        self.service.client.list_shared_paths.side_effect = [
            {"list": [SimpleNamespace(path="/a/1.txt", fs_id=1)]},
            {"list": [SimpleNamespace(path="/a/2.txt", fs_id=2)]},
            {"list": []},
        ]

        pages = list(self.service._iter_shared_dir_pages("/a", 1, 2, "token"))

        self.assertEqual([1, 2, 3], [page for page, _ in pages])
        self.assertEqual(3, self.service.client.list_shared_paths.call_count)

    def test_repeated_page_stops_iteration(self):
        self.service.SHARED_DIR_PAGE_SIZE = 1
        self.service.client.list_shared_paths.side_effect = [
            {"list": [SimpleNamespace(path="/a/1.txt", fs_id=1)]},
            {"list": [SimpleNamespace(path="/a/1.txt", fs_id=1)]},
        ]
...
class SharedPathServiceMetricsTests(unittest.TestCase):
    def setUp(self):
        self.service = SharedPathService(Mock())
        self.service.client = self.service.client

    @patch.dict("os.environ", {"TRANSFERSHARE_STORAGE_METRICS": "1"}, clear=False)
    @patch("storage_shares.emit_storage_metric", side_effect=emit_storage_metric)
    def test_iter_shared_dir_children_emits_shared_page_metrics(self, emit_mock):
        self.service.client.list_shared_paths.side_effect = [
            {"list": [SimpleNamespace(path="/a/1.txt", is_dir=False, is_file=True, fs_id=1)]},
            {"list": []},
        ]

        with patch.object(SharedPathService, "SHARED_DIR_PAGE_SIZE", 1):
            list(self.service.iter_shared_dir_children("/a", 1, 2, "token"))

        events = [call.args[0] for call in emit_mock.call_args_list]
        self.assertIn("shared_dir_pages", events)


class StoragePathServiceMetricsTests(unittest.TestCase):
    @patch.dict("os.environ", {"TRANSFERSHARE_STORAGE_METRICS": "1"}, clear=False)
    @patch("storage_paths.emit_storage_metric", side_effect=emit_storage_metric)
    def test_list_local_files_in_dirs_emits_local_list_metric(self, emit_mock):
        client = Mock()
        client.list.return_value = [
            SimpleNamespace(is_file=True, is_dir=False, path="/save/A/a.txt", md5="md5-a")
        ]
        ...
        service.list_local_files_in_dirs("/save", {"A"}, use_cache=False, merge_dirs=False)

        events = [call.args[0] for call in emit_mock.call_args_list]
        self.assertIn("local_dir_list", events)


class BaiduStorageMetricsFlowTests(unittest.TestCase):
    def setUp(self):
        self.storage = BaiduStorage.__new__(BaiduStorage)
        self.storage.client = Mock()
        self.storage.wechat_notifier = None
    ...
    @patch.dict("os.environ", {"TRANSFERSHARE_STORAGE_METRICS": "1"}, clear=False)
    @patch("storage_transfer_plan.emit_storage_metric", side_effect=emit_storage_metric)
    @patch("storage_transfer_plan.handle_error_and_notify")
    @patch("storage_transfer_plan.time.sleep")
    def test_execute_transfer_plan_emits_rate_limit_sleep_metric(
        self, sleep, notify, emit_mock
    ):
        transfer_item = TransferItem(1, "/save", "a.txt", "a.txt", False, "md5-a")
        self.storage.client.transfer_shared_paths.side_effect = [
            RuntimeError("error_code: -65"),
        ...
        with patch("storage_transfer_plan.RATE_LIMIT_WAIT_TIME", 10):
            self.storage._execute_transfer_plan(
                [transfer_item], "url", 1, 2, "token", "/save"
            )

        events = [call.args[0] for call in emit_mock.call_args_list]
        self.assertIn("rate_limit_sleep", events)


class BatchLocalRecoveryVerificationTests(unittest.TestCase):
    def setUp(self):
        self.storage = BaiduStorage.__new__(BaiduStorage)
        self.storage.client = Mock()
        self.storage.wechat_notifier = None
        self.storage._local_files_cache = {}
        self.storage.path_service = Mock()
        self.storage.path_service.normalize_path.side_effect = lambda path, file_only=False: path
        self.storage.share_service = Mock()

    def test_verify_existing_for_batch_calls_scan_with_batch_parents_only(self):
        items = [
            TransferItem(1, "/save", "A/1.txt", "A/1.txt", False, "md5-a"),
            TransferItem(2, "/save", "B/2.txt", "B/2.txt", False, "md5-b"),
        ]
        self.storage._scan_local_files_dict = Mock(return_value={"A/1.txt": "md5-a"})

        existing, missing = self.storage._verify_existing_for_batch(items, "/save")

        self.assertEqual([items[0]], existing)
        self.assertEqual([items[1]], missing)
        self.storage._scan_local_files_dict.assert_called_once_with("/save", None, {"A", "B"})

    def test_execute_transfer_plan_uses_batch_local_verify_on_failure(self):
        transfer_list = [
            TransferItem(1, "/save", "A/1.txt", "A/1.txt", False, "md5-a"),
            TransferItem(2, "/save", "B/2.txt", "B/2.txt", False, "md5-b"),
        ]
        ...
        self.storage._verify_existing_for_batch = Mock(return_value=([transfer_list[0]], [transfer_list[1]]))
        ...
        self.assertEqual(1, success_count)
        self.assertEqual([transfer_list[0]], successful_items)
        self.assertEqual(1, len(failed_items))
        self.assertEqual(2, failed_items[0]["fs_id"])
        self.storage._verify_existing_for_batch.assert_called_once()


class BatchLocalCacheInvalidationTests(unittest.TestCase):
    def setUp(self):
        self.storage = BaiduStorage.__new__(BaiduStorage)
        self.storage.client = Mock()
        self.storage.wechat_notifier = None
    ...
    def test_verify_existing_for_batch_clears_only_batch_parent_dirs(self):
        items = [
            TransferItem(1, "/save", "A/1.txt", "A/1.txt", False, "md5-a"),
        ]

        self.storage._verify_existing_for_batch(items, "/save", force_refresh=True)

        self.storage._clear_local_files_cache.assert_called_once_with("/save", {"A"})
        self.storage._scan_local_files_dict.assert_called_once_with("/save", None, {"A"})
