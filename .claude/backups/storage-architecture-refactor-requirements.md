...
2. WHEN given a list of transfer candidates and a local files dictionary (mapping normalized paths to MD5 hashes), THE CandidateFilter SHALL return a list of TransferItem objects representing files that need transfer and a summary Counter containing at minimum the keys: existing_count, conflict_count, transfer_needed_count, and rename_needed_count
3. THE CandidateFilter SHALL accept a regex_pattern string, a regex_replace string, and an exclude_folder_filter list at construction time
4. THE CandidateFilter SHALL not depend on BaiduStorage instance state beyond the local files dictionary and configuration passed at construction time
...
5. WHEN internal modules (storage_rename, storage_streaming, storage_transfer_plan) reference attributes on the storage module via "import storage as storage_module", THE system SHALL continue to expose all attributes previously accessed through that pattern
